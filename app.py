...
def get_library_tracks():
  ...
  while True:
    ...
    for track in tracks:
      ...
      db.tracks.find_one_and_update({'track.id': track_id}, {'$set': track}, upsert=True)

    audio_features_list = spotify.tracks.audio_features(track_id=None, ids=tracks_id)

    for audio_features in audio_features_list.get('audio_features'):
      db.tracks.find_one_and_update({'track.id': audio_features.get('id')}, {'$set': {'audio_features':audio_features}}, upsert=False)
